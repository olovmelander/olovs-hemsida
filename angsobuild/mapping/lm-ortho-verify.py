...
reference=ROOT / 'geo_data/course-v2/angso/reference'
...
assert len(plan['windows']) == 19
assert sorted(w['hole'] for w in plan['windows'] if 'hole' in w) == list(range(1, 19))
...
result=dict(schemaVersion=1,groundId='angso',kind='orthophoto-offline-verification',
            ...
            checks=['all expected windows present','raster and RGB checksums','EPSG:3006 and exact planned pixel-edge grid',
                    ...
                    'all 18 complete holes at native resolution','source tile seams coincide with all review pixel grids',
