/* Fuse Ängsö GK's records into one course model.

   OpenStreetMap has mapped four of this course's eighteen holes — with real hole
   numbers on their centre-lines, which is what makes them worth keeping — plus a
   handful of greens, tees and bunkers. GolfTraxx holds only hole 1 of the GPS
   survey. Everything else is read off orthorectified satellite imagery, with the
   club's own banguide supplying the routing, and the four mapped holes standing
   as the check that the traced ones are the right size and in the right places.
