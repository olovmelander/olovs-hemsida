/* Fetch the OpenStreetMap extract for Ängsö and its surroundings.

   The course sits on an island in Mälaren, so the ways that must never arrive
   clipped are the lake shoreline and the island's own coast. The raw map API,
   one core bbox, then re-fetch any tagged way the bbox cut in half.             */
import fs from 'node:fs';
import path from 'node:path';
import { CACHE } from './lib.mjs';

const CORE_BBOX = '16.833,59.550,16.907,59.592';
...
async function get(url, tries = 4) {
  for (let i = 0; i < tries; i++) {
    try {
      const r = await fetch(url, { headers: { 'User-Agent': 'angso3d-angsobuild/1 (+github.com/olovmelander/olovs-hemsida)' } });
