...
const HULL_MIN = 750;
/* Best hull of all: the club's own leisure=golf_course property polygon, when OSM
   has one. Here it is nearly the only golf geometry OSM carries, and it bounds the
   whole 1.6 x 2.0 km estate course that two greens alone would never suggest. */
for (const w of ways.values())
  if (w.tags.leisure === 'golf_course' && /johannesberg/i.test(w.tags.name || '')) coursePts.push(...ptsOf(w));
...
const dest = path.join(ROOT, 'johannesbergbuild', 'osm-features.json');
writeJSON(dest, out);
