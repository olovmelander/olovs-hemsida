"""Acquire georeferenced Lantmateriet RGBI review windows for all 27 Johannesberg holes.
...
they are never written or logged. Raw pixels stay in johannesbergbuild/cache/lm-ortho.
"""
import argparse
import contextlib
...
def make_plan():
    ...
    if discovery['groundId'] != 'johannesberg' or not ortho['coverage']['complete'] or ortho['horizontalCrs'] != 'EPSG:3006':
        raise ValueError('Expected complete Johannesberg EPSG:3006 discovery')
    resolution = ortho['resolutionMetres']
    anchor = ortho['items'][0]['projBbox'][:2]
    transform = Transformer.from_crs(4326, 3006, always_xy=True)
    windows, hole_windows, all_points, models = [], [], [], []
    ...
    for course, filename in [('eighteen', 'johannesbergbuild/course-model.json'), ('nine', 'johannesberg9build/course-model.json')]:
        file = ROOT / filename
        model = json.loads(file.read_text(encoding='utf-8'))
        models.append(dict(course=course, file=filename, sha256=digest(file)))
        def projected(p):
            # Same inverse as johannesbergbuild/lib.mjs, followed by authoritative PROJ.
            origin = model['origin']
        ...
        for hole in model['holes']:
            ...
            points = [p for f in features for p in f['ring']]
            all_points += points + list(map(projected, hole['line']))
            windows.append(window(f'{course}-{hole["n"]:02d}-surfaces', points, 30, resolution,
                                  course=course, hole=hole['n'], purpose='putting surface and all mapped hole bunkers', features=features))
            ...
            hole_windows.append(window(f'{course}-{hole["n"]:02d}-hole', full_points, 40, resolution * 2,
                                       course=course, hole=hole['n'], purpose='complete hole fairways, tees, green and bunker review', features=full_features))
        for ring in model.get('scenery', {}).get('range', []):
            all_points += list(map(projected, ring))
        for ring in model.get('scenery', {}).get('practiceGreens', []):
            all_points += list(map(projected, ring))
    # Existing whole-ground review extent includes the practice range/facilities.
    windows += hole_windows
    ...
    windows.insert(0, window('ground-overview', all_points, 100, resolution*5,
                            purpose='both playing loops, practice ground and surrounding alignment context'))
    ...
    return dict(schemaVersion=1, groundId='johannesberg', kind='authenticated-orthophoto-review-plan',
                ...
                sourceDiscoverySha256=digest(ROOT / 'geo_data/course-v2/johannesberg/acquisition/d2-discovery.json'),
...
def acquire(plan, selected, cache, report_file, probe_only=False):
    ...
    report = dict(schemaVersion=1, groundId='johannesberg', state='access-verified' if access['authorized'] else 'blocked',
    ...
    with rasterio.Env(**environment), contextlib.ExitStack() as stack:
        ...
        for window in selected:
            ...
            for f in window.get('features', []):
                ...
                draw.line(ring+[ring[0]], fill={'green':(30,255,70),'bunker':(255,70,235),'fairway':(255,225,20),'tee':(40,220,255)}[f['kind']], width=3)
...
def main():
    ...
    reference = ROOT / 'geo_data/course-v2/johannesberg/reference/lm-ortho-plan-2026-09-09.json'
    plan = make_plan() if args.replan or not reference.exists() else json.loads(reference.read_text(encoding='utf-8'))
    if plan.get('groundId') != 'johannesberg' or plan.get('horizontalCrs') != 'EPSG:3006':
        raise ValueError('Expected a pinned Johannesberg EPSG:3006 plan')
    ...
    report = cache / 'selected-acquisition.json' if args.only or args.probe_only else ROOT / 'geo_data/course-v2/johannesberg/reference/lm-ortho-acquisition-2026-09-09.json'
    try:
