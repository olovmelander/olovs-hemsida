...
const discovery = JSON.parse(fs.readFileSync(path.join(root, 'geo_data/course-v2/johannesberg/acquisition/d2-discovery.json')));
...
const report = {
  schemaVersion: 1, groundId: 'johannesberg', kind: 'orthophoto-live-catalog-recheck', observedAt: new Date().toISOString(),
  endpoint: STAC_ENDPOINTS.imagery, bboxWgs84: discovery.aoi.bboxWgs84, bboxEpsg3006: discovery.aoi.bboxEpsg3006,
  sourceDiscoverySha256: createHash('sha256').update(fs.readFileSync(path.join(root, 'geo_data/course-v2/johannesberg/acquisition/d2-discovery.json'))).digest('hex'),
...
const output = path.join(root, 'geo_data/course-v2/johannesberg/reference/lm-ortho-catalog-2026-09-09.json');
