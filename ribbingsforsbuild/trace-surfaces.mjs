...
import { readGreen, hull, GREEN_SETS, GREEN_AREA, GREEN_SOLIDITY, GREEN_SHIFT } from './green-grower.mjs';
...
/* ------------------------------------------------------------------ greens */
/* the grower lives in green-grower.mjs so wayback-greens.mjs can run the same
   rule against the 2019 leaf-on release */
const greens = [];
for (const h of model.holes) {
  const { chosen, summary } = readGreen(IMG, T, h.green.c);
  if (chosen) {
