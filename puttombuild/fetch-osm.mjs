/* Fetch the OpenStreetMap extract for Puttom and its surroundings.

   Inland forest course between two lakes at Arnäsvall. The raw map API (Overpass
   resets on large geometry), one core bbox, then re-fetch any tagged way the bbox
   clipped — here the ways that must never arrive clipped are the two lake
   shorelines.                                                                    */
import fs from 'node:fs';
import path from 'node:path';
import { CACHE } from './lib.mjs';

const CORE_BBOX = '18.905,63.283,18.975,63.318';
...
async function get(url, tries = 4) {
  for (let i = 0; i < tries; i++) {
    try {
      const r = await fetch(url, { headers: { 'User-Agent': 'puttom3d-puttombuild/1 (+github.com/olovmelander/olovs-hemsida)' } });
