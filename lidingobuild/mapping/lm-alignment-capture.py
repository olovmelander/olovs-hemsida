"""Intersect published orthophoto seam polygons with each Lidingo review window.
...
plan_file = ROOT / 'geo_data/course-v2/lidingo/reference/lm-ortho-plan-2026-09-09.json'
plan = json.loads(plan_file.read_text(encoding='utf-8'))
catalog = json.loads((ROOT / 'geo_data/course-v2/lidingo/reference/lm-ortho-catalog-2026-09-09.json').read_text(encoding='utf-8'))
sources, footprints = [], []
...
report=dict(schemaVersion=1,groundId='lidingo',kind='orthophoto-window-capture-evidence',observedAt=datetime.now(timezone.utc).isoformat(),
...
out=ROOT / 'geo_data/course-v2/lidingo/reference/lm-ortho-capture-2026-09-09.json'
...
print(json.dumps(dict(windows=len(windows), dates=sorted(set(c['capturedAt'][:10] for w in windows for c in w['contributingImages'])),
                      nativeWindowDates=sorted(set(c['capturedAt'][:10] for w in windows if w['id']!='lidingo-ground-overview' for c in w['contributingImages'])))))
