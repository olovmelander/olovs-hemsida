...
reference=ROOT / 'geo_data/course-v2/lidingo/reference'
...
assert report['planSha256']==capture['planSha256']==sha(plan_file)==sha(cache / 'alignment-plan.json')
...
assert len(plan['windows']) == 37
assert sorted(w['hole'] for w in plan['windows'] if w['id'].endswith('-tees')) == list(range(1, 19))
assert sorted(w['hole'] for w in plan['windows'] if w['id'].endswith('-hole')) == list(range(1, 19))
assert all(w['resolutionMetres'] == plan['nativeResolutionMetres'] for w in plan['windows'] if 'hole' in w)
anchor = plan['sources'][0]['boundsEpsg3006'][:2]
for window in plan['windows']:
    for axis in range(2):
        for bound in (window['boundsEpsg3006'][axis], window['boundsEpsg3006'][axis+2]):
            cell = (bound - anchor[axis]) / window['resolutionMetres']
            assert abs(cell-round(cell)) < 1e-7
        for source in plan['sources']:
            for bound in (source['boundsEpsg3006'][axis], source['boundsEpsg3006'][axis+2]):
                cell = (bound-anchor[axis])/window['resolutionMetres']
                assert abs(cell-round(cell)) < 1e-7

# Overlapping native windows must return the same measured RGBI bytes. This
# catches a crop shifted by one pixel even when its metadata appears valid.
overlap_checks=[]
native=[w for w in report['windows'] if w['resolutionMetres']==plan['nativeResolutionMetres']]
for left,right in combinations(native,2):
    a,b=left['boundsEpsg3006'],right['boundsEpsg3006']
    bounds=[max(a[0],b[0]),max(a[1],b[1]),min(a[2],b[2]),min(a[3],b[3])]
    if bounds[0]>=bounds[2] or bounds[1]>=bounds[3]:
        continue
    with rasterio.open(cache/left['rasterFile']) as src_a, rasterio.open(cache/right['rasterFile']) as src_b:
        wa=from_bounds(*bounds,transform=src_a.transform).round_offsets().round_lengths()
        wb=from_bounds(*bounds,transform=src_b.transform).round_offsets().round_lengths()
        assert np.array_equal(src_a.read(window=wa),src_b.read(window=wb))
        overlap_checks.append(dict(left=left['id'],right=right['id'],pixels=int(wa.width*wa.height),state='identical-RGBI'))

result=dict(schemaVersion=1,groundId='lidingo',kind='orthophoto-offline-verification',
            ...
            nativeOverlapChecks=overlap_checks,
            checks=['all expected windows present','raster and RGB checksums','EPSG:3006 and exact planned pixel-edge grid',
                    ...
                    'all 18 tee corridors and complete holes at native resolution','source tile seams coincide with all review pixel grids',
                    'all overlapping native windows have identical RGBI values'],
...
print(json.dumps({k:result[k] for k in ('state','validatedWindows','nativeWindows','pixels')}))
