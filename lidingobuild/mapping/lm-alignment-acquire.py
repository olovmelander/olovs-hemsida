"""Acquire georeferenced Lantmateriet RGBI review windows for all 18 Lidingo holes.
...
they are never written or logged. Raw pixels stay in lidingobuild/cache/lm-ortho.
"""
import argparse
import contextlib
...
def make_plan():
    # The checked-in evidence plan intentionally pins the pre-review model,
    # including its exact frame and all manually reproducible search extents.
    # Rebuilding from already corrected geometry would destroy that baseline.
    raise ValueError('Use the checked-in Lidingo orthophoto alignment baseline plan')
...
def acquire(plan, selected, cache, report_file, probe_only=False):
    ...
    report = dict(schemaVersion=1, groundId='lidingo', state='access-verified' if access['authorized'] else 'blocked',
                  ...
                  planSha256=digest(cache / 'alignment-plan.json'), access=access, windows=[], sources=sources,
...
def main():
    ...
    reference = ROOT / 'geo_data/course-v2/lidingo/reference/lm-ortho-plan-2026-09-09.json'
    plan = make_plan() if args.replan or not reference.exists() else json.loads(reference.read_text(encoding='utf-8'))
    if plan.get('groundId') != 'lidingo' or plan.get('horizontalCrs') != 'EPSG:3006':
        raise ValueError('Expected a pinned Lidingo EPSG:3006 plan')
    ...
    write_json(cache / 'alignment-plan.json', plan)
    ...
    report = cache / 'selected-acquisition.json' if args.only or args.probe_only else ROOT / 'geo_data/course-v2/lidingo/reference/lm-ortho-acquisition-2026-09-09.json'
    try:
