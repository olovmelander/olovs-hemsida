...
const teeReview = json('lidingobuild/mapping/tee-native-alignment-review-2026-09-09.json');
...
const report = {
  ...
  newOrthophoto: { accessState: orthophotoAudit.state, measuredFeatures: orthophotoAudit.features.length,
    geometryVerificationFromImagery: 'tee platforms visually reviewed; other playing surfaces pending',
    boundaryVerification: 'Native 2025 tee review supersedes the historical spectral review queue for tees only.',
    teeReviewPath: 'lidingobuild/mapping/tee-native-alignment-review-2026-09-09.json',
    teeCaptureDate: teeReview.captureDate, reviewedTeeHoles: teeReview.holes.map(h => h.hole),
    unresolvedColourReferences: teeReview.holes.flatMap(h => h.markers.filter(m => m.unresolved).map(m => ({ hole: h.hole, reason: m.reason }))) },
  surfaceRecords,
};
