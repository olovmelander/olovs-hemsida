...
/* Put the hålguide's per-hole text into the committed model.
   ...
   Same reason `apply-tee-marks.mjs` and `apply-vista-landcover.mjs` exist:
   `build-course.mjs` cannot run in a checkout without the acquired 1 m raster
   it pins by sha256, so the committed model is updated through the generator's
   OWN exported rule -- `holeNotes` -- and `course.node-test.mjs` re-derives it
   a third time and demands equality. No second implementation.

     node visbybuild/mapping/apply-guide-notes.mjs [--write] */
