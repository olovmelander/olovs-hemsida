...
test('all 108 references are reviewed, with unresolved H12 and altered H3 first', () => {
  ...
  assert.equal(plan.summary.unresolvedTeeReferences, 33);
  assert.deepEqual(plan.windows.find(w => w.id === 'hole-14-tees').unresolvedTees, ['63', '59'], 'membership on turf must not hide unresolved numbered identity');
