...
function insertCandidate(candidates, candidate) {
  // Keep the same stable priority order without sorting the whole queue for
  // every child. Equal candidates follow existing entries, as stable sort did.
  let low = 0, high = candidates.length;
  while (low < high) {
    const middle = (low + high) >>> 1;
    if (candidateOrder(candidate, candidates[middle]) < 0) high = middle;
    else low = middle + 1;
  }
...
export class TerrainTileManager {
  constructor({ ground, courseSlug } = {}) {
    ...
    // Verified hierarchy is fixed for this manager's lifetime. Reuse ancestry
    // during planning; camera, visibility, residency and errors stay live.
    this.ancestorPaths = new Map();
    for (const tile of this.tiles.values()) {
      const path = [];
      let cursor = tile;
      while (cursor) { path.push(cursor); cursor = this.tiles.get(this.parentById.get(cursor.id)); }
      this.ancestorPaths.set(tile.id, Object.freeze(path));
    }
  ...
  plan({
    ...
    for (const tile of desired) {
      const path = this.ancestorPaths.get(tile.id);
      for (let index = path.length - 1; index >= 0; index--) {
        const pathTile = path[index];
        if (resident.has(pathTile.id)) continue;
    ...
    for (const tile of desired) {
      for (const ancestor of this.ancestorPaths.get(tile.id)) retain.add(ancestor.id);
    }
