/* Real course check: Hero/Impostor routing, model requests, empty retired tiers,
 * slot ownership, and a rendered screenshot. SwiftShader proves correctness,
...
const expectedModels = catalogue.species.flatMap(s => s.variants.map(v => v.tiers.hero.file)).sort();
...
try {
  ...
  console.log('Tee rendered; Hero-only model requests and drawable inventory verified.');
  ...
  report.ok = true;
  console.log(`PASS ${course} / ${quality}: ${report.views.map(v => `${v.view} ${v.tiers.tier0} Hero, ${v.tiers.tier3} Impostor`).join('; ')}`);
} catch (error) {
