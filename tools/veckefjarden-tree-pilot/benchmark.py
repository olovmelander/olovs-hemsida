...
def main():
    ...
    freeze=read(DOC/'reference-freeze.json');assert digest(DOC/'reference.geojson')==freeze['sha256']
    if stage=='calibration':scenes=[s for s in scenes if s['split']=='calibration']
    ...
    lock=dict(selected=selected,selectionUses='Calibration only; all four evaluation windows excluded from selection',referenceSha256=digest(DOC/'reference.geojson'),matchingToleranceMetres=MATCH_METRES,methods=methods)
    if stage=='calibration':
        save(DOC/'detector-selection.json',lock);save(DOC/'calibration-benchmark.json',metrics)
        print(json.dumps(dict(selected=selected,calibration=metrics[selected]['calibration']),indent=2));return
    assert lock==read(DOC/'detector-selection.json'),'Selection must remain frozen before evaluation'
