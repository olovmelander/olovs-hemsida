...
import {OUT,serve,save,json} from './round4-preview.mjs';
const round='round4';
...
const servers={before:await serve('round3'),after:await serve(round||'after')};
...
try{
 ...
 for(let repeat=0;repeat<repeats;repeat++)for(const mode of repeat%2?['after','before']:['before','after']){
  ...
  await page.evaluate(()=>{V3D.setPreset('noon');V3D.goHole(16,true,true);V3D.setCam('tee',true)});
  await page.waitForFunction(()=>V3D.settled(),null,{timeout:60000});await page.waitForTimeout(1000);
  const priorGround=runs.find(r=>r.mode===mode&&r.scenario===scenario.name)?.state;
  const state=await page.evaluate(({priorGround})=>{
   const trees=V3D.legacyTrees({instances:true});const residuals=priorGround?null:trees.instances.map(a=>Math.abs(a[1]+.25-V3D.terrainH(a[0],a[2]))).sort((a,b)=>a-b);
   return {backend:V3D.stats.backend,quality:V3D.quality(),perf:V3D.perf(),objects:V3D.v2Objects(),audit:V3D.treeTierAudit(),catalogue:V3D.treeCatalogue(),
    drawn:trees.total,groundResidual:priorGround?{...priorGround.groundResidual,reusedFromRepeat:0}:{sampleCount:residuals.length,median:residuals[Math.floor(residuals.length/2)],p95:residuals[Math.floor(residuals.length*.95)],max:residuals.at(-1)}};
  },{priorGround});
  if(priorGround){assert.equal(state.drawn,priorGround.drawn);assert.deepEqual(state.catalogue,priorGround.catalogue);assert.equal(state.objects.loaded.records,priorGround.objects.loaded.records);}
  ...
  runs.push({mode,scenario:scenario.name,repeat,bootMs,frames,state,errors});save(path.join(work,reportPrefix+'-runs.json'),{browser:browser.version(),runs});
...
save(path.join(work,reportPrefix+'-summary.json'),{browser:browser.version(),repeats,hole:16,camera:'tee',viewport:[1440,900],comparison,
 comparisonModes:{before:'round three',after:'round four'},
