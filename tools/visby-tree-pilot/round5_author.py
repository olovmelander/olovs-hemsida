"""Apply explicit full-facility extension decisions to the retained fourth preview."""
from round5 import *
from stand_fields import mask_geometries
corridor=extension

def main():
    check_lock();path=ROOT/'tools/visby-tree-pilot/round5-decisions.json';decisions=read(path)
    ...
    for scene in read(WORK/'review-scenes.json'):
        ...
        assert d['treatment'] in ['open','individuals','woodland','mixed','uncertain'],sid
        ...
        for f in local:
            ...
            if action=='accept':
                ...
                record=dict(id=id,easting=x,northing=y,objectHeightMetres=h,radiusMetres=radius,sourceId='visby-tree-pilot-round5-source-review',placementMethod='derived-lidar',
    ...
    save(DOC/'corrections.json',dict(version=5,facilityScopeSha256=digest(DOC/'facility-scope.geojson'),groundId='visby',status='local-experiment-only',reviewer=decisions['reviewer'],
    ...
    for job in read(PREVIOUS/'stand-output/index.json'):
        ...
        jobs.append(dict(job,changed=job['changed'] or altered,round5Changed=altered,sha256=digest(target),canopyAreaMetres2=float(np.sum(new[:,:,0]/255)*job['cellMetres']**2)))
    ...
    for f in coverage['features']:
        ...
        p.update(status='ambiguous' if issues else 'reviewed',inspected=True,reviewer=decisions['reviewer'],reviewDate='2026-09-16',note=d['note'],treatment=d['treatment'],issueIds=issues,
