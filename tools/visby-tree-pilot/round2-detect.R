...
scenes <- fromJSON(file.path(if(stage=="fresh")work else root,"scenes.json"),simplifyVector=FALSE)
if(stage!="fresh")scenes<-Filter(function(s)s$split=="calibration",scenes)
source <- rast(file.path(root,"chm.tif"))
dir.create(file.path(work,"detections"),recursive=TRUE,showWarnings=FALSE)
for(scene in scenes){
 ...
 for(size in c(3,5)){
  ...
  for(base in c(5,7,9)){
   ...
   for(method in c("dalponte","silva")){
    ...
    writeRaster(fn(),file.path(work,"detections",paste0(scene$id,"-",name,".tif")),overwrite=TRUE,datatype="INT4S",NAflag=-1)
   }
  }
 }
...
write_json(list(stage=stage,R=R.version.string,lidR=as.character(packageVersion("lidR")),terra=as.character(packageVersion("terra")),
 algorithms="lidR Dalponte and Silva",minimumHeight=3,windowDiameters="clamp(base + 0.1h, 3, 14)",bases=c(5,7,9),medianSizes=c(3,5),haloMetres=20),
 file.path(work,paste0(stage,"-tool-run.json")),pretty=TRUE,auto_unbox=TRUE)
