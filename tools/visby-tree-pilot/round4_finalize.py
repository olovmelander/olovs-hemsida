...
def main():
    ...
    assert len(raw)==18
    for r in raw:
        assert r['state']['objects']['loaded']['records']==(3230 if r['mode']=='before' else 3368)
    ...
    for br,ar in zip(read(PREVIOUS/'captures/report.json')['runs'],read(WORK/'captures/report.json')['runs']):
        for av in ar['views']:
            ...
            pairs.append(dict(scenario=ar['scenario'],hole=av['hole'],camera=av['cam'],beforeFile='round3/captures/'+bv['file'],
                afterFile='round4/captures/'+av['file'],beforeSha256=bv['sha256'],afterSha256=av['sha256'],cameraIdentical=bv['camera']==av['camera']))
    ...
    files=sorted((ROOT/'tools/visby-tree-pilot').glob('round4*'))
    files += [ROOT/'tools/visby-tree-pilot'/name for name in ['round4-compile.mjs','round4-preview.mjs','round4-performance.mjs']]
