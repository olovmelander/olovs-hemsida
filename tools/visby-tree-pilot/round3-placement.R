# Frozen round-two settings; no parameter selection on these review cells.
root <- "output/visby-tree-pilot"
work <- file.path(root,"round3")
...
for(scene in fromJSON(file.path(work,"review-scenes.json"),simplifyVector=FALSE)){
 ...
 if(nrow(tops)==0){ labels<-chm;values(labels)<-NA } else {
...
write_json(list(R=R.version.string,lidR=as.character(packageVersion("lidR")),terra=as.character(packageVersion("terra")),
 method=lock$method,maximumIslandAreaMetres2=lock$maximumIslandAreaMetres2,haloMetres=20,
 policy="Reuse previously frozen settings for source review only; not a new detector evaluation"),file.path(work,"tool-run.json"),pretty=TRUE,auto_unbox=TRUE)
