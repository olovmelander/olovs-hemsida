...
import {OUT,serve,save} from './preview.mjs';
const {server,url}=await serve('round3');
const browser=await chromium.launch({channel:'chrome'});
try{
 const page=await browser.newPage({viewport:{width:1440,height:1100},serviceWorkers:'block'}),errors=[];
 page.on('pageerror',e=>errors.push(String(e)));
 const prefix=url+'/pilot-review/round3/';await page.goto(prefix+'review.html',{waitUntil:'networkidle'});
 assert.equal(await page.locator('#scene option').count(),34);
 ...
 assert.equal(await page.locator('#accepted').innerText(),'61');
 ...
 await page.locator('#map path').evaluateAll(paths=>paths.find(p=>p.querySelector('title')?.textContent?.startsWith('r3-'))?.dispatchEvent(new MouseEvent('click')));
 ...
 await page.selectOption('#renderer','webgpu-high');await page.selectOption('#hole','1');await page.selectOption('#scene','r3-04');
 const assets=await page.evaluate(()=>[...PILOT3.scenes.flatMap(s=>['rgb','cir','chm','2022'].map(l=>`review/${s.id}-${l}.png`)),
 ...
 assert.equal(errors.length,0);await page.screenshot({path:path.join(OUT,'round3/review-page.png'),fullPage:true});
 save(path.join(OUT,'round3/review-page-check.json'),{status:'passed',browser:browser.version(),scenes:34,sourceLayers:4,checkedLocalAssets:assets.length,placementPanelMetres:130,suppressionPanelMetres:30,errors});
 console.log('Review page passed:',assets.length,'assets; 34 scenes; source extents and comparisons verified.');
