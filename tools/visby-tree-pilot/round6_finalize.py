...
def main():
    ...
    for br,ar in zip(read(PREVIOUS/'captures/report.json')['runs'],read(WORK/'captures/report.json')['runs']):
        for av in ar['views']:
            ...
            pairs.append(dict(scenario=ar['scenario'],hole=av['hole'],camera=av['cam'],beforeFile='round5/captures/'+bv['file'],
                afterFile='round6/captures/'+av['file'],beforeSha256=bv['sha256'],afterSha256=av['sha256'],cameraIdentical=bv['camera']==av['camera']))
    ...
    closeups=read(WORK/'closeups.json');assert len(closeups['pairs'])==18
    for p in closeups['pairs']:
        for mode in ['before','after']:assert digest(WORK/'closeups'/p[mode])==p[mode+'Sha256']
    save(DOC/'closeups.json',closeups)
    files=sorted((ROOT/'tools/visby-tree-pilot').glob('round6*'))
    files += [ROOT/'tools/visby-tree-pilot'/name for name in ['round6-compile.mjs','round6-preview.mjs','round6-performance.mjs']]
    files += [WORK/'runtime-probes.json',WORK/'gap-probe-points.json',WORK/'captures/report.json',WORK/'closeups.json',WORK/'review-scenes.json',WORK/'review-data.js']
