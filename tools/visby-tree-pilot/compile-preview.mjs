// Emit only into output/visby-tree-pilot/after. The live root is an invariant.
import fs from 'node:fs';
import path from 'node:path';
import assert from 'node:assert/strict';
import {OUT,ROOT,PUBLIC,json,save,sha} from './preview.mjs';
...
if(round)evidence.previousPublicationSha256=sha(fs.readFileSync(path.join(OUT,round==='round3'?'round2':'','publication.json')));
