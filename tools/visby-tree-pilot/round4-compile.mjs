// Emit only into output/visby-tree-pilot/round4/after. The live root is an invariant.
import fs from 'node:fs';
import path from 'node:path';
import assert from 'node:assert/strict';
import {OUT,ROOT,PUBLIC,json,save,sha} from './round4-preview.mjs';
...
if(round)evidence.previousPublicationSha256=sha(fs.readFileSync(path.join(OUT,'round3','publication.json')));
