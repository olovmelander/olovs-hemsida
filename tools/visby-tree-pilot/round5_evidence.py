...
def summarize_coverage(features):
    ...
    summary=dict(groundId='visby',review='round5',scopeAreaMetres2=area.area,gridCells=len(features),inspectedCells=sum(bool(f['properties'].get('inspected')) for f in features),
        ...
        definition='Cumulative systematic source/candidate and rendered-base review, clipped to the reviewed facility and excluding six protected evaluation windows. Ambiguous cells were inspected but did not pass. Woodland interiors remain density-based, not a stem census.',
...
def main():
    ...
    for c in cats:
        if c['action'].startswith('hold'):
            issues.append(dict(id='r4-'+c['candidateId'],group='round5-'+c['action'],status='unresolved',geometry=c['geometry'],reason=c['reason'],relatedRecordId=c.get('id'),scene=c['scene'],hole=c['hole'],relatedIssueIds=[]))
    for c in read(ROOT/'tools/visby-tree-pilot/round5-decisions.json')['additionalObservations']:
        meta=read(WORK/'review'/(c['scene']+'.json'));w,s,e,n=meta['bounds'];px,py,r=c['pixels']
        owned=next(shape(f['geometry']) for f in features if f['properties'].get('scene')==c['scene']).intersection(extension()).difference(reserve)
        g=Point(w+px/650*(e-w),n-py/650*(n-s)).buffer(r/650*(e-w)).intersection(owned)
        assert not g.is_empty and g.area>0,c['id']
        issues.append(dict(id=c['id'],group='round5-low-vegetation',status='unresolved',geometry=mapping(g),reason=c['reason'],relatedRecordId=None,scene=c['scene'],hole=meta['hole'],relatedIssueIds=[]))
    if (DOC/'canopy-gap-issues.json').exists():issues += read(DOC/'canopy-gap-issues.json')['cases']
    ...
    save(DOC/'issue-index.json',dict(caseCount=len(issues),groupCounts=dict(Counter(c['group'] for c in issues)),cases=issues,
        definition='Unresolved review cases, not unique trees. Prior cases retained; new source/base holds and low-vegetation observations added. Spatial overlap is a possible relationship, not proof of identical stems.',
    ...
    ax.set_title(f"Visby · full-facility review\n{summary['inspectedCells']} cells inspected · {summary['inspectedFraction']:.1%} of facility area\nAmber areas retain unresolved cases; this is not a tree census",fontsize=12)
