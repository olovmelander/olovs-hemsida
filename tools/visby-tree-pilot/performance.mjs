...
assert(!(process.argv.includes('--round2')&&process.argv.includes('--round3')),'Choose one round');
const round=process.argv.includes('--round3')?'round3':process.argv.includes('--round2')?'round2':'';
const work=round?path.join(OUT,round):OUT;
...
try{
 ...
 for(let repeat=0;repeat<repeats;repeat++)for(const mode of repeat%2?['after','before']:['before','after']){
  ...
  const state=await page.evaluate(()=>{
   const trees=V3D.legacyTrees({instances:true});const residuals=trees.instances.map(a=>Math.abs(a[1]+.25-V3D.terrainH(a[0],a[2]))).sort((a,b)=>a-b);
