...
def main():
    ...
    else:unresolved=[shape(c['geometry']) for c in read(PRIOR_DOC/'issue-index.json')['cases']]
    ...
    summary=dict(mode='round4' if baseline else 'round5',scopeAreaMetres2=scope.area,sourcePixelMetres=1,scopePixelCount=int(inside.sum()),finiteHeightPixelCount=int(np.sum(inside&np.isfinite(height))),
