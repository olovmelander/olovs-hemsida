...
def main():
    ...
    assert len(lookup)==len(new)==len(old),'Record IDs/count changed'
    edited={e['id']:e for e in read(DOC/'corrections.json')['edits']}
    for r in old:
        if r['id'] not in edited:assert lookup[r['id']]==r,'Unaffected record changed'
        else:
            e=edited[r['id']];assert e['before']==r and e['after']==lookup[r['id']]
            assert all(lookup[r['id']][k]==v for k,v in r.items() if k not in ['radiusMetres','sourceId']),'Unapproved record field changed'
    cat=read(DOC/'corrections.json');accepted=[]
    ...
    gaps += [shape(f['geometry']) for name in ['round3','round4'] for f in read(PRIOR_DOC.parent/name/'clearings.geojson')['features']]
    ...
    for scenario in ['webgpu-high','webgl-high','webgl-low']:
        ...
        prior_export=read(PREVIOUS/'captures'/(scenario+'-instances.json'))
        before_instances=prior_export['instances'];after_instances=export['instances'];scale_checks=[]
        assert len(before_instances)==len(after_instances),'Unexpected population change'
        modified_indices=set()
        for id,e in edited.items():
            r=e['after'];x=r['easting']-origin['easting'];z=origin['northing']-r['northing']
            i=min(range(len(after_instances)),key=lambda i:(after_instances[i][0]-x)**2+(after_instances[i][2]-z)**2)
            a=after_instances[i];b=before_instances[i];assert a[6]==5 and np.hypot(a[0]-x,a[2]-z)<.02
            assert a[:8]==b[:8] and a[8]>b[8],'Only crown width may change'
            expected=e['radiusAfterMetres']/e['radiusBeforeMetres'];observed=a[8]/b[8]
            assert abs(expected-observed)<.003,'Rendered radius disagrees with record'
            modified_indices.add(i);scale_checks.append(dict(id=id,expectedScaleRatio=expected,renderedScaleRatio=observed))
        assert all(a==b for i,(a,b) in enumerate(zip(after_instances,before_instances)) if i not in modified_indices),'Unreviewed instance changed'
        ...
        results[scenario]=dict(drawn=export['total'],individuals=len(individual),standRepresentatives=len(stands),missingAccepted=missing,
            standBasesInsideIndividualFootprints=duplicates,basesInPreviousClearings=intrusions,basesInNewClearings=new_intrusions,crownScaleChecks=scale_checks,unaffectedInstancesUnchanged=True)
    ...
    report=dict(status='inspection' if inspect else 'passed',previousRecords=len(old),records=len(new),acceptedAdditionalCrowns=len(accepted),
        ...
        unaffectedRecordsUnchanged=True,allBasePositionsAndIdsUnchanged=True,resizedCrowns=len(edited),productionAndPreviousRootsUnchanged=True,freshEvaluationUnedited=True,freshStandCellsVerified=fresh_cells,
        changedStandCells=changed_cells,matchedScreenshotPairs=views,rendered=results,newClearingHeightChecks=gap_support,
        detectorTargetMet=read(DETECTOR_DOC/'evaluation.json')['passed'],newDetectorEvaluation=False)
