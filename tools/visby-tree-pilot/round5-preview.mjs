...
export async function serve(mode='round5',port=0) {
  if(!['before','after','round2','round3','round4','round5'].includes(mode)) throw Error('Expected before, after, round2 or round4');
  const roots=[path.join(OUT,/^round[2345]$/.test(mode)?`${mode}/after`:mode),path.join(OUT,'build'),path.join(OUT,'before'),PUBLIC];
...
export async function capture(mode='round5',quick=false,onlyScenario) {
  ...
  const directory=/^round[2345]$/.test(mode)?path.join(OUT,mode,'captures'):path.join(OUT,'captures',mode);fs.mkdirSync(directory,{recursive:true});
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
  const [command,mode='round5',port]=process.argv.slice(2);
  if(command==='serve'){const result=await serve(mode,Number(port)||8649);console.log(result.url+'/?bana=visby&v2=require');}
  ...
  else throw Error('Usage: round5-preview.mjs serve round4|round5 [port] | capture round5 [--quick]');
}
