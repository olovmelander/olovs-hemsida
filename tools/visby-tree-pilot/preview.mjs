...
export function freeze() {
  if (fs.existsSync(path.join(OUT,'baseline.json'))) throw Error('Baseline already frozen; use the existing snapshot');
  const root=json(path.join(PUBLIC,'courses/v2-index.json'));
  const entry=root.courses.find(c=>c.slug==='visby');
  const course=json(path.join(PUBLIC,entry.manifest.url));
  const ground=json(path.join(PUBLIC,course.groundManifest.url));
  const files=new Set(['courses/v2-index.json','courses/index.json',entry.manifest.url,course.groundManifest.url,
    course.routing.url,ground.shell.url,entry.fallbackV1.packUrl]);
  const startup=json(path.join(PUBLIC,'courses/index.json')).courses.find(c=>c.slug==='visby')?.startup;
  if(startup){files.add(startup.url);for(const pack of json(path.join(PUBLIC,startup.url)).packs)files.add(pack.url);}
  for (const tile of ground.tiles) for (const ref of Object.values(tile.layers)) if(ref) files.add(ref.url);
  const identities=[];
  for(const url of files) {
    const bytes=fs.readFileSync(path.join(PUBLIC,url)),target=path.join(OUT,'before',url);
    fs.mkdirSync(path.dirname(target),{recursive:true});fs.writeFileSync(target,bytes);
    identities.push({url,bytes:bytes.length,sha256:sha(bytes)});
  }
  fs.cpSync(path.join(PUBLIC,'models/trees'),path.join(OUT,'before/models/trees'),{recursive:true});
  ...
  save(path.join(OUT,'baseline.json'),{version:1,capturedAt:new Date().toISOString(),entry,course,ground,identities,records:records.length});
  console.log(`Frozen ${records.length} trees and ${files.size} source files`);
}
...
export async function serve(mode='before',port=0) {
  if(!['before','after','round2','round3'].includes(mode)) throw Error('Expected before, after, round2 or round3');
  const roots=[path.join(OUT,/^round[23]$/.test(mode)?`${mode}/after`:mode),path.join(OUT,'build'),path.join(OUT,'before'),PUBLIC];
...
export async function capture(mode='before',quick=false) {
  ...
  const directory=/^round[23]$/.test(mode)?path.join(OUT,mode,'captures'):path.join(OUT,'captures',mode);fs.mkdirSync(directory,{recursive:true});
  const runs=[];
  try {
    for(const scenario of [{name:'webgpu-high',query:'&q=hi'}, {name:'webgl-high',query:'&gl=1&q=hi'}, {name:'webgl-low',query:'&gl=1&q=lo'}]) {
      ...
      runs.push(run);save(path.join(directory,'report.json'),{mode,runs});await context.close();
    }
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
  const [command,mode='before',port]=process.argv.slice(2);
  if(command==='freeze')freeze();
  else if(command==='serve'){const result=await serve(mode,Number(port)||8644);console.log(result.url+'/?bana=visby&v2=require');}
  else if(command==='capture')await capture(mode,process.argv.includes('--quick'));
  else throw Error('Usage: preview.mjs freeze | serve before|after|round2|round3 [port] | capture before|after|round2|round3 [--quick]');
}
