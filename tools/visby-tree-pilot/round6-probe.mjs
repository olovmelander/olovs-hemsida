// Read round five at diagnostic gap centres; these are not proposed trunk positions.
import path from 'node:path';
import {chromium} from 'playwright-core';
import {OUT,ROOT,serve,json,save} from './round5-preview.mjs';
const work=path.join(OUT,'round6');
const baseline=json(path.join(OUT,'baseline.json'));
const absent=[];
const candidates=json(path.join(work,'gap-probe-points.json'));
