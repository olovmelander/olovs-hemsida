...
try {
  ...
  assert.equal(await page.locator('.bag-save').textContent(),'Klar','an unchanged bag offers Klar');
  await page.locator('[data-club-id="iron-7"] .bag-distance').fill('147');
  assert.equal(await page.locator('.bag-save').textContent(),'Spara ändringar','a change is announced on the button');
  // Enter moves to the next carry; it used to submit through the close button.
  await page.keyboard.press('Enter');
  assert.equal(await page.locator('#bagDialog').evaluate(el=>el.open),true,'Enter keeps the editor open');
  assert.equal(await page.evaluate(()=>document.activeElement.closest('.bag-row')?.dataset.clubId),'iron-8');
  ...
  assert.ok(await page.locator('#bagConfirm').isVisible(),'closing with changes asks first');
  await page.keyboard.press('Escape');
  assert.ok(await page.locator('#bagConfirm').isHidden(),'Escape in the question keeps editing');
  assert.equal(await page.locator('[data-club-id="iron-7"] .bag-distance').inputValue(),'170');
  await page.locator('#bagCloseBtn').click();await page.locator('#bagDiscardBtn').click();
  await page.evaluate(()=>bagEditor.open());
  assert.equal(await page.locator('[data-club-id="iron-7"] .bag-distance').inputValue(),'147');
  await page.locator('[data-club-id="iron-9"] .bag-distance').fill('400');
  await page.locator('.bag-save').click();
  assert.ok(await page.locator('#bagError').isVisible(),'an impossible carry blocks saving');
  assert.equal(await page.evaluate(()=>document.activeElement.closest('.bag-row')?.dataset.clubId),'iron-9');
  await page.locator('[data-club-id="iron-9"] .bag-distance').fill('120');
  await page.locator('[data-club-id="iron-7"] .bag-preview').click();
  await page.locator('#clubRemove').click();
  assert.equal(await page.locator('[data-club-id="iron-7"]').count(),0);
  await page.locator('#bagUndoBtn').click();
  assert.equal(await page.locator('[data-club-id="iron-7"]').count(),1,'undo restores a removed club');
  await page.locator('[data-club-id="iron-7"] .bag-preview').click();
  await page.locator('#clubRemove').click();
  await page.locator('#bagAddBtn').click();
  await page.locator('#clubNameInput').fill('Min favorit');
  await page.locator('#clubKind input[value=wedge]').check();
  await page.locator('.bag-save').click();
  assert.equal(await page.evaluate(()=>savedBag.find(c=>c.name==='Min favorit').kind),'wedge');
  ...
  await page.locator('.bag-inventory').evaluate(el=>el.scrollTop=0);
  ...
  assert.ok(await page.locator('#bagDetail').evaluate(el=>el.inert),'a phone shows the list alone');
  await page.locator('[data-club-id="iron-7"] .bag-preview').click();
  await page.locator('#clubCanvas[data-state=ready]').waitFor();
  assert.ok(await page.locator('.bag-inventory').evaluate(el=>el.inert),'a club opens as its own page');
  await page.waitForTimeout(750);await page.screenshot({path:path.join(OUT,'bag-mobile-club.png')});
  await page.locator('#clubBack').click();
  assert.equal(await page.locator('#bagDialog').getAttribute('data-view'),'list');
  await page.locator('.bag-save').click();
  ...
  report.interactions=['all 14 models','head and full views','face preset','sole preset returns to head view','keyboard rotation','drag rotation','save','Enter moves to the next carry','unsaved-change question','discard','invalid carry blocks save','add/remove','undo remove','model type persistence','reset','putter carry excluded','reduced motion','mobile list and club page'];
  ...
  // A phone opens on the list and downloads no model until a club is opened.
  const phone=await browser.newPage({viewport:{width:390,height:844},deviceScaleFactor:1,isMobile:true,hasTouch:true});
  const models=[];phone.on('request',r=>{if(r.url().endsWith('.glb'))models.push(r.url());});
  phone.on('pageerror',e=>errors.push(e.message));
  await phone.goto('http://127.0.0.1:5186/');await phone.locator('#bagDialog[open]').waitFor();await phone.waitForTimeout(500);
  assert.deepEqual(models,[],'the phone list loads no club model');
  await phone.locator('[data-club-id="driver"] .bag-preview').tap();
  await phone.locator('#clubCanvas[data-state=ready]').waitFor({timeout:30000});
  assert.equal(models.length,1);
  await phone.close();
  report.interactions.push('phone defers the model');
