/* Fetch the OpenStreetMap extract for Upsala GK and its surroundings.

   Parkland/farmland west of Uppsala. The raw map API, one core bbox, then
   re-fetch any tagged way the bbox cut in half — here the ponds and the
   watercourse through the property.                                            */
import fs from 'node:fs';
import path from 'node:path';
import { CACHE } from './lib.mjs';

const CORE_BBOX = '17.458,59.820,17.535,59.860';
...
async function get(url, tries = 4) {
  for (let i = 0; i < tries; i++) {
    try {
      const r = await fetch(url, { headers: { 'User-Agent': 'upsala3d-upsalabuild/1 (+github.com/olovmelander/olovs-hemsida)' } });
