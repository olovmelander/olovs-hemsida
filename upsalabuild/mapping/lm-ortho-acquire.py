"""Acquire georeferenced Lantmateriet RGBI review windows for both Upsala loops.
...
they are never written or logged. Raw pixels stay in upsalabuild/cache/lm-ortho.
"""
import argparse
import contextlib
...
def make_plan():
    ...
    discovery = json.loads((ROOT / 'geo_data/course-v2/upsala/acquisition/d2-discovery.json').read_text(encoding='utf-8'))
    ortho = discovery['orthophoto']
    if discovery['groundId'] != 'upsala' or not ortho['coverage']['complete'] or ortho['horizontalCrs'] != 'EPSG:3006':
        raise ValueError('Expected complete Upsala EPSG:3006 discovery')
    ...
    for course, filename in [('stora', 'upsalabuild/course-model.json'), ('mellan', 'upsalamellanbuild/course-model.json')]:
        file = ROOT / filename
        model = json.loads(file.read_text(encoding='utf-8'))
        models.append(dict(course=course, file=filename, sha256=digest(file)))
        def projected(p):
            # Same inverse as upsalabuild/lib.mjs, followed by authoritative PROJ.
            origin = model['origin']
    ...
    all_points += [[639119.5, 6635121.5], [641167.5, 6637169.5]]
    ...
    return dict(schemaVersion=1, groundId='upsala', kind='authenticated-orthophoto-review-plan',
                ...
                sourceDiscoverySha256=digest(ROOT / 'geo_data/course-v2/upsala/acquisition/d2-discovery.json'),
...
def acquire(plan, selected, cache, report_file, probe_only=False):
    ...
    report = dict(schemaVersion=1, groundId='upsala', state='access-verified' if access['authorized'] else 'blocked',
    ...
    with rasterio.Env(**environment), contextlib.ExitStack() as stack:
        ...
        for window in selected:
            ...
            with rasterio.open(temp, 'w', driver='GTiff', width=window['width'], height=window['height'],
                               count=4, dtype='uint8', crs=3006, transform=trans, compress='deflate', tiled=True,
                               photometric='RGB', ALPHA='UNSPECIFIED') as dst:
            ...
            for f in window.get('features', []):
                ...
                draw.line(ring+[ring[0]], fill=(30,255,70) if f['kind']=='green' else (255,70,235), width=3)
...
def main():
    ...
    cache = ROOT / 'upsalabuild/cache/lm-ortho'
    reference = ROOT / 'geo_data/course-v2/upsala/reference/lm-ortho-plan-2026-09-09.json'
    plan = make_plan() if args.replan or not reference.exists() else json.loads(reference.read_text(encoding='utf-8'))
    if plan.get('groundId') != 'upsala' or plan.get('horizontalCrs') != 'EPSG:3006':
        raise ValueError('Expected a pinned Upsala EPSG:3006 plan')
    ...
    report = cache / 'selected-acquisition.json' if args.only or args.probe_only else ROOT / 'geo_data/course-v2/upsala/reference/lm-ortho-acquisition-2026-09-09.json'
    try:
