...
for record in report['windows']:
    ...
    if record['id']!='ground-overview':
        assert date['captureRange']['first'].startswith('2025-06-14') and date['captureRange']['last'].startswith('2025-06-14')
    pixels+=record['width']*record['height']

result=dict(schemaVersion=1,groundId='upsala',kind='orthophoto-offline-verification',
