"""Acquire georeferenced Lantmateriet RGBI review windows for all 18 Norrfallsviken holes.
...
they are never written or logged. Raw pixels stay in nvgkbuild/cache/lm-ortho.
"""
import argparse
import contextlib
...
def make_plan():
    ...
    discovery = json.loads((ROOT / 'geo_data/course-v2/norrfallsviken/acquisition/d2-discovery.json').read_text(encoding='utf-8'))
    catalog = json.loads((ROOT / 'geo_data/course-v2/norrfallsviken/reference/lm-ortho-catalog-2026-09-09.json').read_text(encoding='utf-8'))
    # The broad discovery includes 2012 imagery over offshore sea. Every
    # review window here must be covered by the newest RGBI campaign alone.
    items = [i for i in catalog['items'] if i['collection'] == catalog['collection']]
    if discovery['groundId'] != 'norrfallsviken' or not items or any(i['projCode'] != 'EPSG:3006' or i['spectralType'] != 'rgbi' for i in items):
        raise ValueError('Expected latest Norrfallsviken EPSG:3006 RGBI discovery')
    resolution = items[0]['resolutionMetres']
    if any(i['resolutionMetres'] != resolution for i in items):
        raise ValueError('Latest campaign has mixed native resolution')
    ortho = dict(collection=catalog['collection'], items=items,
                 captureRange=dict(first=min(i['captureStart'] for i in items), last=max(i['captureEnd'] for i in items)))
    ...
    def window(identifier, points, margin, output_resolution, **details):
        ...
        if not unary_union([box(*i['projBbox']) for i in items if i['id'] in ids]).covers(box(*bounds)):
            raise ValueError('Latest RGBI campaign does not completely cover review window')
    ...
    all_features = []
    for course, filename in [('norrfallsviken', 'nvgkbuild/course-model.json')]:
        file = ROOT / filename
        model = json.loads(file.read_text(encoding='utf-8'))
        models.append(dict(course=course, file=filename, sha256=digest(file)))
        def projected(p):
            # Same inverse as nvgkbuild/lib.mjs, followed by authoritative PROJ.
            origin = model['origin']
    ...
    for identifier, corners, purpose in [
        ('facilities-range', [[-455, 80], [-60, 345]], 'clubhouse roof, terrace, practice greens, range, hitting pads, paths and parking'),
        ('storsanden-beach', [[-640, -1050], [50, -610]], 'beach, sea edge, dunes, forest edge and beach trails'),
        ('east-coast-north', [[180, -650], [650, -20]], 'northern east coast, sea, forest edge and boundary paths'),
        ('east-coast-south', [[180, -20], [650, 610]], 'southern east coast, sea, forest edge and boundary paths'),
    ]:
        points = list(map(projected, corners))
        all_points += points
        windows.append(window(identifier, points, 0, resolution, purpose=purpose, features=[]))
    ...
    return dict(schemaVersion=1, groundId='norrfallsviken', kind='authenticated-orthophoto-review-plan',
                ...
                sourceDiscoverySha256=digest(ROOT / 'geo_data/course-v2/norrfallsviken/acquisition/d2-discovery.json'),
                sourceCatalogSha256=digest(ROOT / 'geo_data/course-v2/norrfallsviken/reference/lm-ortho-catalog-2026-09-09.json'),
                models=models, sources=sources, windows=windows,
                localFrameInverse='lon=18.53250+x/(111320*cos(62.98250 degrees)); lat=62.98250-z/111320; project WGS84 to EPSG:3006 with PROJ',
...
def acquire(plan, selected, cache, report_file, probe_only=False):
    ...
    report = dict(schemaVersion=1, groundId='norrfallsviken', state='access-verified' if access['authorized'] else 'blocked',
...
def main():
    ...
    reference = ROOT / 'geo_data/course-v2/norrfallsviken/reference/lm-ortho-plan-2026-09-09.json'
    plan = make_plan() if args.replan or not reference.exists() else json.loads(reference.read_text(encoding='utf-8'))
    if plan.get('groundId') != 'norrfallsviken' or plan.get('horizontalCrs') != 'EPSG:3006':
        raise ValueError('Expected a pinned Norrfallsviken EPSG:3006 plan')
    ...
    report = cache / 'selected-acquisition.json' if args.only or args.probe_only else ROOT / 'geo_data/course-v2/norrfallsviken/reference/lm-ortho-acquisition-2026-09-09.json'
    try:
