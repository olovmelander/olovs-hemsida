#!/usr/bin/env python3
"""Build the tree-cover raster for the Mjällom cape from Esri z17 imagery.
...
remainder, which on this cape IS forest: the imagery is high summer, so conifer
and deciduous canopy are both green, dark and violently textured, while the
heath and sandy waste between the corridors is pale, brown and only mildly
textured.
...
Z = 17
LAT0, LON0 = 62.98250, 18.53250
M_PER_LON = 111320.0 * math.cos(math.radians(LAT0))

# ---------------------------------------------------------------- raster frame
CELL = 3.0
X0, Z0 = -1500.0, -1500.0
NX = int((1200 - X0) / CELL) + 1
NZ = int((1600 - Z0) / CELL) + 1
...
print("wrote nvgkbuild/tree-cover.json")
