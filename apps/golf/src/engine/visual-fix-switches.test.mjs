...
describe('the visual fixes\' before switches', () => {
  ...
  it('leave prepared startup eligible for the landscape and air batches\' befores, and all of them together', () => {
    for (const allowed of [preparedTintAllowed, preparedWaterAllowed, preparedVistaAllowed, preparedScatterAllowed]) {
      for (const flag of [...LANDSCAPE_BEFORE, ...AIR_BEFORE]) expect(allowed(`?bana=angso&${flag}`)).toBe(true);
      expect(allowed(`?bana=angso&${[...BEFORE, ...LIGHTING_BEFORE, ...LANDSCAPE_BEFORE, ...AIR_BEFORE].join('&')}`)).toBe(true);
    }
  });
});
