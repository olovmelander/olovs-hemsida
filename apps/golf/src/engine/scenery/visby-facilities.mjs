/* Visby assets use the same EPSG:3006 local frame as the terrain. Installation
 * is atomic: failed or cancelled loads leave the source buildings and tower.
 * Each top-level facility keeps its absolute RH2000 mesh heights; the runtime
 * applies its height bridge exactly once, without rescaling the architecture. */
export const VISBY_CLUBHOUSE_SOURCE_ID = 'way/530655631';
export const VISBY_LIGHTHOUSE_SOURCE_ID = 'skansudde-lighthouse';
...
const sourceKey = facility => facility.sourceBuildingId ?? facility.sourceLandmarkId ?? facility.sourceFeatureId;
const sourceField = facility => facility.sourceBuildingId ? 'sourceBuildingId'
  : facility.sourceLandmarkId ? 'sourceLandmarkId' : 'sourceFeatureId';
...
const sourceRing = (facility, buildings) => facility.sourceBuildingId
  ? buildings.get(facility.sourceBuildingId)?.ring : facility.sourceLandmarkId
    ? [[-608.3, 185.4], [-598.3, 185.4], [-598.3, 195.4], [-608.3, 195.4]]
    : facility.footprintLocal;

export function validateVisbyFacilitiesManifest(manifest, { courseSlug, buildings }) {
  assert(courseSlug === 'visby', 'Facilities belong to another course');
  assert(manifest?.schemaVersion === 1 && manifest.groundId === 'visby', 'Invalid facilities manifest identity');
  assert(Array.isArray(manifest.courseSlugs) && manifest.courseSlugs.length === 1
    && manifest.courseSlugs[0] === 'visby', 'Facilities course mismatch');
  const frame = manifest.coordinateFrame;
  assert(frame?.kind === 'epsg3006-local-rh2000' && frame.originEpsg3006?.easting === 687748.5
    && frame.originEpsg3006?.northing === 6370951.5 && frame.axes === 'east-up-south'
  ...
  assert(manifest.facilities.some(f => f.sourceBuildingId === VISBY_CLUBHOUSE_SOURCE_ID),
  ...
  for (const facility of manifest.facilities) {
    ...
    assert([facility.sourceBuildingId, facility.sourceLandmarkId, facility.sourceFeatureId]
      .filter(value => value !== undefined).length === 1 && typeof source === 'string' && source.length > 0
      && !sources.has(source), 'Invalid facility source');
    if (facility.sourceBuildingId) assert(bySource.has(source), 'Replacement building is unavailable');
    if (facility.sourceLandmarkId) assert(source === VISBY_LIGHTHOUSE_SOURCE_ID, 'Unknown replacement landmark');
    ...
    assert(bounds.minX >= -2048 && bounds.maxX <= 2048 && bounds.minZ >= -2048 && bounds.maxZ <= 2048,
      'Facility footprint leaves Visby');
...
export function visbyFacilityVerticalPlacement(facility, { v2Active, terrainH, verticalDatumOffsetMetres }) {
...
function localAssetUrl(path, baseUrl) {
  ...
  assert(url.origin === base.origin && url.pathname.startsWith(base.pathname.replace(/\/?$/, '/') + 'models/visby/'),
...
export async function loadVisbyFacilities({ THREE, scene, courseSlug, buildings, terrainH,
  ...
  const replacedBuildingIds = new Set(), replacedLandmarkIds = new Set();
  ...
  const report = { status: 'loading', courseSlug, replacedBuildingIds: [], replacedLandmarkIds: [],
  ...
  const dispose = () => {
    ...
    replacedLandmarkIds.clear();
  };
  const current = () => assert(!controller.signal.aborted && isCurrentCourse(), 'Facilities load was cancelled');
  try {
    current();
    assert(courseSlug === 'visby', 'Facilities belong to another course');
    const response = await fetchImpl(localAssetUrl('models/visby/facilities-v1.json', baseUrl), { signal: controller.signal });
    assert(response.ok, 'Facilities manifest could not be loaded');
    const manifest = validateVisbyFacilitiesManifest(await response.json(), { courseSlug, buildings });
    current();
    ...
    root.traverse(object => {
      // GLTFLoader sanitizes spaces/punctuation in Object3D.name for animation
      // bindings and retains the exact authored glTF node name in userData.
    ...
    for (const facility of manifest.facilities) {
      ...
      assert(bounds.min.x >= footprint.minX - 30 && bounds.max.x <= footprint.maxX + 30
        ...
        && bounds.min.y > -10 && bounds.max.y < 100 && bounds.max.y - bounds.min.y > .01,
      'Facility bounds leave their source footprint');
      const placement = visbyFacilityVerticalPlacement(facility, { v2Active, terrainH, verticalDatumOffsetMetres });
    ...
    current();
    root.name = 'Visby authored facilities';
    root.userData = { ...root.userData, groundId: 'visby', assetSha256: hash, facilityCount: manifest.facilities.length };
    scene.add(root);
    root.updateMatrixWorld(true);
    for (const facility of manifest.facilities) {
      if (facility.sourceBuildingId) replacedBuildingIds.add(facility.sourceBuildingId);
      if (facility.sourceLandmarkId) replacedLandmarkIds.add(facility.sourceLandmarkId);
    }
    report.status = 'loaded';
    report.assetSha256 = hash;
    report.replacedBuildingIds = [...replacedBuildingIds];
    report.replacedLandmarkIds = [...replacedLandmarkIds];
    return { report, replacedBuildingIds, replacedLandmarkIds, root, dispose };
  } catch (error) {
    dispose();
    Object.assign(report, { status: 'fallback', reason: error instanceof Error ? error.message : 'Facilities unavailable',
      replacedBuildingIds: [], replacedLandmarkIds: [], facilities: [], meshes: 0, triangles: 0 });
    return { report, replacedBuildingIds, replacedLandmarkIds, root: null, dispose };
  } finally {
