...
export function paintedGround({ base, wp, DETAIL, uSun, mow = float(0), turf = float(1), seasonal = float(0), gloss = null }) {
  ...
  let blotch = blotchA.mul(0.18).add(blotchB.mul(0.08)).add(blotchC.mul(0.035)).mul(mix(float(0.35), float(1), turf));
...
const PAINTED_SHADING = {
  ...
  finishV2({ material, litBase, wp, DETAIL, uSun, mow, meta, seasonal, shade, surfaceGloss = false }) {
    Object.assign(material, paintedGround({ base: litBase, wp, DETAIL, uSun, mow,
      turf: oneMinus(meta.g.max(meta.b)), seasonal, gloss: surfaceGloss ? shade.z : null }));
  },
};
